...
func newUMI(f string) map[int]string {
	fh, err := os.Open(f)
	if err != nil {
		fmt.Println("%v\n", err)
		os.Exit(1)
...
func main() {
	...
	fout1, _ := os.OpenFile(prefix + "_final_1.fq", os.O_CREATE|os.O_WRONLY, 0666)
	defer fout1.Close()

	fout2, _ := os.OpenFile(prefix + "_final_2.fq", os.O_CREATE|os.O_WRONLY, 0666)
	defer fout2.Close()
	...
	fumi := filepath.Join(exPath, "randomseq.txt")
	umis    := newUMI(fumi)
	umi_cts := make(map[string]int)

	qual1 := strings.Repeat("F", 30)
	...
	for r, done := fqr.Iter(); !done; r, done = fqr.Iter() {
		...
		if _, indb := bc_keep[bc]; indb {
			...
			qual := r.Qual[28:]
			new_id := r.Name + "_" + bc
                        fout2.WriteString(new_id+"\n"+seq+"\n+\n"+qual+"\n")
			umi_cts[bc] += 1

			n := umi_cts[bc]
			new_umi := umis[n]
			seq1 := bc + new_umi
                        fout1.WriteString(new_id+"\n"+seq1+"\n+\n"+qual1+"\n")
